...
nonisolated struct CallAnalyticsInventory {
    ...
    private var skillRoots: [(source: CallSourceKind, path: String)] {
        [
            (.claude, "\(environment["CLAUDE_CONFIG_DIR"] ?? homeDirectory + "/.claude")/skills"),
            (.codex, "\(environment["CODEX_HOME"] ?? homeDirectory + "/.codex")/skills"),
            (.opencode, "\(environment["XDG_CONFIG_HOME"] ?? homeDirectory + "/.config")/opencode/skills"),
            // 只列出可确定归属 Pi 的全局技能；不将其他客户端技能算作 Pi 的零调用项。
            (.pi, PiSessionPaths.agentDirectory(homeDirectory: homeDirectory, environment: environment) + "/skills")
        ]
    }
    ...
    private var mcpConfigFiles: [(source: CallSourceKind, path: String, format: MCPConfigFormat)] {
        [
            ...
            (.codex, "\(environment["CODEX_HOME"] ?? homeDirectory + "/.codex")/config.toml", .codexTOML)
        ]
    }
    ...
    func installedMCPServers() -> [InstalledItem] {
        ...
        // 仅提取配置中的服务器名称；凭据、参数和环境变量不进入 InstalledItem。
        var openCodeNames = Set<String>()
        let configRoot = environment["XDG_CONFIG_HOME"] ?? homeDirectory + "/.config"
        let paths = [configRoot + "/opencode/opencode.json", environment["OPENCODE_CONFIG"]].compactMap { $0 }
        for path in paths { collectJSONMCPNames(path: path, into: &openCodeNames) }
