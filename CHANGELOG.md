...
- 修复刷新期间删除或禁用账号后，旧额度、订阅和账号列表重新出现的问题；连续操作多个账号均可正确保存。
- 修复 Codex 禁用账号仍发起请求的问题，统一多来源账号识别，并避免同邮箱不同账号的额度混淆。
- 加强 Claude 外部凭据保护，避免刷新或覆盖 CLI、钥匙串和 Desktop 管理的令牌；认证失效时保留上次成功额度。
- 修复菜单栏提供商筛选时菜单重建导致的交互问题，切换筛选保持菜单项稳定。
- 兼容 Amp 新版金额与订阅额度格式，改进 Kiro 区域校验和 Antigravity 令牌过期时间解析。
- 修复预热错误过早消失、取消 IDE 刷新后仍写回结果，以及代理版本回退失败时原版本文件过早清理的问题。
- 选择性移植上游 v0.32.0 及其后的适用修复，保留 QuotioPlus 当前架构、工作区管理和统计功能。

详细说明：[QuotioPlus 1.0.03](docs/releases/1.0.03.md)。
...
- **技能更新时间修复**：修复已纳管技能在某些情况下错误显示“更新于 1年1月1日”的缺陷，改为获取文件系统的实际修改时间，若无有效时间则隐藏。
- **仪表盘体验重构**：将“性能与吞吐”独立置顶（形态 B），脱离 Cap 统计联动；移除不适用的用量折线图，精简与统一用量分布筛选器联动。
- **客户端与模型配置**：优化 Claude Code / Codex 模型映射与默认模型显示，修复会话内 `/model` 面板标题显示，增强 SQLite 用量状态与安全检查。
- **更新体验优化**：更新机制优化为直接跳转至 GitHub Releases 发布页手动下载，彻底停用 Sparkle 原生弹窗与错位界面。
...
- 构建 74 覆盖重打包：最低 macOS 15.0，仅发布 Apple Silicon（arm64），更新通道隔离旧 Intel 客户端。
- 请求、历史汇总、价格列表整页滚动并固定每页 20 条；默认窗口显示主要列，宽窗口补充次要指标。
- 默认窗口仪表盘指标一行四等分，窄窗口继续两列；优化图表悬停与大数据绘制开销。
