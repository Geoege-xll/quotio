...
- `[des-dashboard-003] [hard]` 总览使用四个主指标内嵌卡：请求数、Tokens、成功率、平均延迟；默认窗口优先一行四等分，每张占扣除间距后的 1/4 宽度（2026-09-08 用户确认）。宽度不足沿用现有自适应布局，变为每行两张，极窄时单列；主卡最小列宽为 120pt。紧凑窗口中的文字、字号、数值格式及卡片内部处理逻辑沿用原样。图标默认彩色，数字使用系统主文本色。十三项次级指标在一个轻量底面内分成 Token 用量、缓存、性能与吞吐、请求结果四组，名称和值同行、名称左对齐、数值右对齐。次级指标组在默认窗口也必须一行四等分，最小列宽为 120pt，保留 16pt 组间距及 12pt 底面内边距；宽度不足时两列，极窄时单列。全部指标直接展示，不再设置“更多指标”，不重复同一指标。
- `[des-dashboard-004] [hard]` 已知正数下界使用“≥”，未知数值使用“—”；总体分母无法确定时不能显示精确比率。历史日汇总与单次请求分开显示，不用旧日桶虚构请求时间、缓存拆分或请求详情。
- `[des-dashboard-005] [hard]` 价格统计和请求明细采用整页纵向滚动，筛选、摘要、说明、表格与分页底栏一起移动（2026-09-08 用户确认）。单次请求、历史日汇总、模型价格固定每页最多 20 条，通过上一页／下一页翻页，不能无限滚动或扩大页容量。顶部统计基于完整筛选结果；保留原生表格的横向滚动与列宽调整，内容不得撑大窗口。
- `[des-dashboard-006] [hard]` 默认窗口优先完整显示主要列，不依赖横向滚动。价格表默认显示模型、请求数、Tokens、费用，次要单价在宽窗口展开；请求表默认显示时间、模型、结果、Tokens、延迟，提供商放入模型次级文字，详情入口与模型同行。历史表默认显示日期、模型（含提供商）、请求数、Tokens。保留长文本和费用的悬停全文，完整请求指标和单价分别在详情、编辑面板中可读。
